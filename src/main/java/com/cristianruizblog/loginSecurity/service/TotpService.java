...
import com.cristianruizblog.loginSecurity.totp.TOTP;
...
public class TotpService {
	...
	private String generateTotpBySecret(String secret) {
		...
		// Encoding time frame value to HEX string - requred by TOTP generator which is
		// used here.
		String timeEncoded = Long.toHexString(timeFrame);

		String totpCodeBySecret;
		try {
			// Encoding given secret string to HEX string - requred by TOTP generator which
			// is used here.
			char[] secretEncoded = (char[]) new Hex().encode(secret);

			// Generating TOTP by given time and secret - using TOTP algorithm
			// implementation provided by IETF.
